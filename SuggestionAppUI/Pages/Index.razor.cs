...
public partial class Index
{
  ...
  private async Task LoadAndVerifyUser()
  {
    ...
    if (string.IsNullOrWhiteSpace(objectId) == false)
    {
      ...
      if (email.Equals(loggedInUser.FirstName) == false)
      {
        isDirty = true;
        loggedInUser.FirstName = email;
      }

      if (email.Equals(loggedInUser.LastName) == false)
      {
        isDirty = true;
        loggedInUser.LastName = email;
      }

      if (email.Equals(loggedInUser.DisplayName) == false)
      {
        isDirty = true;
        loggedInUser.DisplayName = email;
      }
  ...
  {
    List<SuggestionModel> output = await suggestionData.GetAllApprovedSuggestions();
  ...
  private async Task ArchiveSuggestion()
  {
    ...
    _suggestions.Remove(archivingSuggestion);
